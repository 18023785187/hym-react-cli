...
const config_stylus = async () => {
    const process = ora(chalk.blue(`⚙️  config stylus\n`))
    process.start()
    process.color = "yellow"
    process.text = chalk.blue(`⚙️  config stylus\n`)
...
const config_cesium = async () => {
    ...
    const process = ora(chalk.blue(`⚙️  config cesium\n`))
    process.start()
    process.color = "yellow"
    process.text = chalk.blue(`⚙️  config cesium\n`)
...
const config_dll = async (vendor) => {
    ...
    fs.readFile(`./package.json`, 'utf8', async (err, data) => {
        ...
        const process = ora(chalk.blue(`⚙️  config dll\n`))
        process.start()
        process.color = "yellow"
        process.text = chalk.blue(`⚙️  config dll\n`)
